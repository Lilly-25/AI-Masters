...
def create_sudoku_tensors(df):
    ...
    quizzes_t = df.partialsudokus.apply(one_hot_encode)
    solutions_t = df.filledsudokus.apply(one_hot_encode)
    quizzes_t = torch.cat(quizzes_t.values.tolist())
    solutions_t = torch.cat(solutions_t.values.tolist())
    randperm = torch.randperm(s)
    train = randperm[:int(0.8 * s)]
    test = randperm[int(0.2 * s):]
...
def load_dataset(subsample=10000):
    dataset = pd.read_csv("/home/lilly/Studies/ANN/train_sudokus.csv", sep=',')
    #my_sample = dataset.sample(subsample)
    train_set, test_set = create_sudoku_tensors(dataset)
